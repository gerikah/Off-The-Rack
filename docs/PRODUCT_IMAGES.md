...
2. Confirm `supabase/migrations/003_product_images.sql` has already been applied, then apply `supabase/migrations/007_product_image_workflow.sql` to development/staging and production. Migration 007 keeps the existing table, normalizes legacy cover flags, enforces at most one primary image, and hardens the existing bucket policies. Do not replay migration 003 when it is already present.
...
5. Keep `sharp` as a production dependency. `next.config.ts` allows a 40 MB Server Action request body so up to 12 browser-prepared images (3 MB maximum each) plus multipart overhead can accompany a new product.

## Administrator workflow

- On **Add product**, select several images at once before saving. The form previews each file, shows its name, removes selected files on request, and ignores content-identical duplicates. Product validation runs before the product row is created. The first uploaded image is the cover and all rows receive sequential `sort_order` values. If any create-time image step fails, the incomplete product and successful earlier uploads are rolled back or queued for safe cleanup.
- On **Edit product**, existing images load in saved order. Select several additional images and use **Upload selected images**; successful additions append without replacing existing rows. Image additions save independently of text fields, and a text-only save never mutates `product_images`.
...
`npm run test -- tests/image-upload.spec.ts tests/admin-images.spec.ts tests/admin.spec.ts` checks signature/MIME and size validation, real decoder failures, JPEG/PNG/WebP/AVIF conversion, resizing and metadata removal, multi-image create/append/order/primary behavior, orphan rollback, plus the admin replace/remove/alt/shared-reference flow and membership recheck against the local backend fixture. Existing admin CRUD/status/category/inquiry tests remain in place.
